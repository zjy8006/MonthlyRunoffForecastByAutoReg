...
vmd_err_aps = pd.DataFrame(vmd_err_aps.values,columns=[r'Cal errors for VMD $IMF_{1}$'])
vmd_sc_val_error = pd.DataFrame(vmd_sc_val_error,columns=[r'Val errors for VMD $IMF_{1}$'])
dwt_err_aps = pd.DataFrame(dwt_err_aps.values,columns=[r'Cal errors for DWT $D_{1}$'])
dwt_sc_val_error = pd.DataFrame(dwt_sc_val_error,columns=[r'Val errors for DWT $D_{1}$'])
eemd_err_aps = pd.DataFrame(eemd_err_aps.values,columns=[r'Cal errors for EEMD $IMF_{1}$'])
eemd_sc_val_error = pd.DataFrame(eemd_sc_val_error,columns=[r'Val errors for EEMD $IMF_{1}$'])
ssa_err_aps = pd.DataFrame(ssa_err_aps.values,columns=[r'Cal errors for SSA $S_{1}$'])
ssa_sc_val_error = pd.DataFrame(ssa_sc_val_error,columns=[r'Val errors for SSA $S_{1}$'])
...
for i in range(len(data)):
    ...
    data[i].plot(kind='kde',color='purple',linewidth=2,ax=ax[i])
plt.tight_layout()
