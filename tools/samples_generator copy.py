...
def gen_concurrent_samples(inputs,outputs,lags_dict):
    input_columns = list(inputs.columns)
    # Get the number of input features
    subsignals_num = inputs.shape[1]
    # Get the data size
    ...
    for i in range(subsignals_num):
        ...
        one_in = (inputs[input_columns[i]]).values  # subsignal
...
def gen_wddff_samples(data_path,lead_time,lag,use_full,dev_len=120,mode='MI',threshold=0.1,n_components=None):
    ...
    normalize_indicators.to_csv(save_path+'norm_unsample_id.csv',index=None)
...
def gen_direct_hindcast_samples(
    ...
    if mode == 'PACF' and n_components == None:
        save_path = data_path+"one_step_" + \
            str(lead_time)+"_ahead_hindcast_pacf/"
    elif mode == 'PACF' and n_components != None:
        save_path = data_path+"one_step_" + \
            str(lead_time)+"_ahead_hindcast_pacf_pca"+str(n_components)+"/"
    elif mode == 'Pearson' and n_components == None:
        save_path = data_path+"one_step_" + \
            str(lead_time)+"_ahead_hindcast_pearson"+str(filter_boundary)+"/"
    elif mode == 'Pearson' and n_components != None:
        save_path = data_path+"one_step_" + \
            str(lead_time)+"_ahead_hindcast_pearson" + \
    ...
    if len(os.listdir(save_path))>0 and not regen:
        ...
        if mode == 'PACF':
            ...
            assert samples_size-lead_time+1 == full_samples.shape[0]
            # Get the training and developing set
            ...
            assert (train_samples['X1'].size + dev_samples['X1'].size +
                    test_samples['X1'].size) == samples_size-lead_time+1
            if n_components != None:
                ...
                for i in range(1, pca_X.shape[1]+1):
                    columns.append('X'+str(i))
                pca_X = pd.DataFrame(pca_X, columns=columns)
                logger.info("Input features after PAC:\n{}".format(pca_X.tail()))
                pca_samples = pd.concat([pca_X, y], axis=1)
            ...
            for col in input_columns:
                ...
                logger.info("Entire pearson coefficients:\n{}".format(corrs))
                corrs = (corrs[output_column]).iloc[0:corrs.shape[0]-1]
                orig_corrs = corrs.squeeze()
                logger.info("Selected pearson coefficients:\n{}".format(orig_corrs))
                bools = abs(orig_corrs) >= filter_boundary
                logger.info("Conditions judge:{}".format(bools))
            ...
            logger.info("Selected inputs:\n{}".format(selected))
            logger.info("Entire inputs:\n{}".format(input_df.head()))
            columns = []
            for i in range(0, input_df.shape[1]):
                columns.append("X"+str(i+1))
            columns.append("Y")

            samples = pd.DataFrame((pd.concat([input_df, orig], axis=1)).values, columns=columns)
...
def gen_direct_forecast_samples_triandev_test(station, decomposer, lags_dict, input_columns, output_column, start, stop, test_len,
...
def gen_direct_forecast_samples(station, decomposer, lags_dict, input_columns, output_column, start, stop, test_len,
...
def gen_multi_hindcast_samples(station, decomposer, lags_dict, columns, test_len,
                                    wavelet_level="db10-2", lead_time=1,regen=False):
    """ 
    ...
    if len(os.listdir(save_path))>0 and not regen:
        ...
        for k in range(len(columns)):
            ...
            normalize_indicators.to_csv(
                save_path+'norm_unsample_id_s'+str(k+1)+'.csv')
            train_samples.to_csv(
                save_path+'minmax_unsample_train_s'+str(k+1)+'.csv', index=None)
            dev_samples.to_csv(
                save_path+'minmax_unsample_dev_s'+str(k+1)+'.csv', index=None)
            test_samples.to_csv(
                save_path+'minmax_unsample_test_s'+str(k+1)+'.csv', index=None)


def gen_multi_forecast_samples(station, decomposer, lags_dict, columns, start, stop, test_len, wavelet_level="db10-2", lead_time=1,regen=False):
    """ 
