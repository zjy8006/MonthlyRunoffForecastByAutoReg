...
graphs_path=parent_path+'\\graph\\'

print('Current path:{}'.format(root_path))
print('Parent path:{}'.format(parent_path))
print('Grand path:{}'.format(grandpa_path))
...
def plot_trend_abrupt(timeSeries, confidence, start_year, end_year, series_name, fig_savepath):
    ...
    plt.ylabel('records(' + r'$10^8m^3$' + '/s)', fontsize=10)
    ...
    plt.savefig(fig_savepath, format='EPS',  dpi=1200)
    plt.show()
...
def plot_abrupt(timeSeries, **kwargs):
    ...
    * `ax`: [`Axes`]:
    ...
    ax.set_ylabel('UF~UB', fontsize=10)
...
def plot_trend(timeSeries, **kwargs):
    ...
    * `ax`: [`Axes`]:
    ...
    if fig_id is None:
        ...
        ax.set_xlabel('Time(Year)\n('+fig_id+')', fontsize=10)
    ax.set_ylabel('records(' + r'$10^8m^3$' + '/s)', fontsize=10)
    ax.plot(years, timeSeries,c="b",label=series_name)
    ax.plot(years, linear_trend, '--',c="r", label='Linear trend')
    ax.legend(
        loc=0,
        # bbox_to_anchor=(0.22,1.005, 1,0.1005),
        # ncol=3,
...
if __name__ == '__main__':
    Yangxian = pd.read_excel(parent_path+'/time-series/YangXianRunoff1967-2014.xlsx', sheet_name='Year')['AnnualRunoff']
    zhangjiashan = pd.read_excel(parent_path+'/time-series/ZhangJiaShanRunoff1967-2017.xlsx', sheet_name='Year')['AnnualRunoff']
    ...
    plt.savefig(graphs_path+'trand_abrupt.eps',format='EPS',dpi=1000)
    plt.show()
