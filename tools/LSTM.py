...
def BuildLSTM(train_samples, dev_samples, test_samples, norm_id, model_path, lags=None, seed=None,
    ...
    # Config path to save the optimal results
    ...
    def build_model(hp):
        ...
        for i in range(num_layers):
            ...
            if i == 0:
                model.add(layers.LSTM(units=units, activation='relu',return_sequences=True, input_shape=input_shape))
            else:
        ...
        model.compile(
            optimizer=keras.optimizers.Adam(
                hp.Float('learning_rate', min_value=min_learnrate, max_value=max_learnrate, sampling='LOG', default=1e-2)),
    ...
    # Predict the data set without retraining the model parameters
    best_model = tuner.get_best_models(num_models=1)[0]
    train_predictions_0 = best_model.predict(train_x).flatten()
    dev_predictions_0 = best_model.predict(dev_x).flatten()
    test_predictions_0 = best_model.predict(test_x).flatten()

    # Retrain the model parameters with the training and development samples
    ...
    if retrain or not os.path.exists(checkpoint_path):
        history = model.fit(X, y, epochs=n_epochs, batch_size=batch_size, validation_data=(
            X, y), verbose=1, callbacks=[cp_callback, early_stopping, ])
    ...
    train_predictions_0 = np.multiply(train_predictions_0 + 1, sMax - sMin) / 2 + sMin
    dev_predictions_0 = np.multiply( dev_predictions_0 + 1, sMax - sMin) / 2 + sMin
    test_predictions_0 = np.multiply(test_predictions_0 + 1, sMax - sMin) / 2 + sMin
    ...
    if cast_to_zero:
        ...
        train_predictions_0[train_predictions_0 < 0.0] = 0.0
        dev_predictions_0[dev_predictions_0 < 0.0] = 0.0
        test_predictions_0[test_predictions_0 < 0.0] = 0.0
    ...
    dump_pred_results(
        path=opt_path+'/opt_pred_0.csv',
        train_y=train_y,
        train_predictions=train_predictions_0,
        dev_y=dev_y,
        dev_predictions=dev_predictions_0,
        test_y=test_y,
        test_predictions=test_predictions_0,
        time_cost=time_cost,
    )
    ...
    plot_rela_pred(train_y, train_predictions_0, measurement_time=measurement_time,measurement_unit=measurement_unit, fig_savepath=opt_path + 'TRAIN-PRED-0.png')
    plot_rela_pred(dev_y, dev_predictions_0, measurement_time=measurement_time, measurement_unit=measurement_unit, fig_savepath=opt_path + "DEV-PRED-0.png")
    plot_rela_pred(test_y, test_predictions_0, measurement_time=measurement_time, measurement_unit=measurement_unit, fig_savepath=opt_path + "TEST-PRED-0.png")
    plot_error_distribution(test_predictions_0, test_y,opt_path+'TEST-ERROR-DSTRI-0.png')
