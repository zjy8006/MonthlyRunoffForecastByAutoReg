...
% using EEMD for to provide inputs for operational forecasting tasks.
clear;
close all;
clc;
...
save_path = '../boundary_effect/eemd-decompositions-huaxian/';
...
%% some sample parameters for EEMD             
M = 100; % The number of ensemble members
epsilon = 0.2; % The white noise amplitude 
...
figure('Name','Test shift-invariancy for EEMD');
hold on
...
hold off
x0_imf1_2_791 = x0_dec.IMF1(2:791,1);
x1_imf1_1_790 = x1_dec.IMF1(1:790,1);
...
% Perform EEMD on original and extended time series.
x_1_791_imf = eemd(x_1_791,epsilon,M);
[m,n] = size(x_1_791_imf);
columns = {};
for i=1:n
    if i==1
        columns{i}='ORIG';
...
x_1_792_imf = eemd(x_1_792,epsilon,M);
[m,n] = size(x_1_792_imf);
columns = {};
for i=1:n
    if i==1
        columns{i}='ORIG';
...
% additional observation has on impact on EEMD)
...
% Perform EEMD on the original and extended time series.
x_1_552_imf = eemd(x_1_552,epsilon,M);
[m,n] = size(x_1_552_imf);
columns = {};
for i=1:n
    if i==1
        columns{i}='ORIG';
